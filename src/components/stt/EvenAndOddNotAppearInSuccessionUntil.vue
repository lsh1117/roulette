<template>
	<div class="EvenAndOddNotAppearInSuccessionUntil">
		<div class="accordion accordion-area">

			<div class="accordion-header" @click="toggleAccordion">
				<label class="label-medium">짝수/홀수 현재까지 연속 미등장 횟수</label>
						...
						<li class="chart-item" v-for="(item, index) in numberStats" :key="index">
							<div class="chart-bar">
								...
								<div class="chart-bar-label">
									<span class="ball-645" :class="'ball-' + groupColor[item.number-1]">{{ groupLabels[item.number-1] }}</span>
								</div>
								<!-- 연속 등장 횟수와 바 차트 -->
								<div class="chart-bar-volum">
									<span class="chart-bar-bg" :style="{ width: item.count * 0.5 + 'px' }"></span>
...
<script setup>
    ...
    // 구간 텍스트
    const groupLabels = ref(["짝수","홀수"])
    const groupColor = ref(["even","odd"])

	// 아코디언 상태 (열림/닫힘)
	const isOpen = ref(false);

	// 전체 데이터에서 연속 미등장 횟수 계산
	const appearInSuccession = () => {
		const draws = drwStore.getNumbers(); // Store에서 당첨 번호들 가져오기
		return drwStore.getEvenAndOddNotAppearInSuccessionUntil(draws); // 연속 미등장 횟수 계산
	};
